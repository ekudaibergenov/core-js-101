...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const [a1, b1] = a > b ? [b, a] : [a, b];

  return `${startBracket}${a1}, ${b1}${endBracket}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = ccnStr.length - 1; i >= 0; i -= 2) {
    const valPrev = Number(ccnStr[i]);
    const val = Number(ccnStr[i - 1] || 0) * 2;

    if (val > 9) sum += Number(String(val)[0]) + Number(String(val)[1]) + valPrev;
    else sum += val + valPrev;
  }
...
function getDigitalRoot(num) {
  ...
  if (sum > 9) return String(sum).split('').reduce((acc, item) => acc + Number(item), 0);
...
const isBracketsBalanced = (str) => {
  ...
  const openBrackets = Object.keys(dict);
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBrackets.includes(str[i])) stack.push(str[i]);
      ...
      if (str[i] === dict[stack[stack.length - 1]]) stack.pop();
      else return false;
    }
  }
...
function getCommonDirectoryPath(paths) {
  ...
  const commonPath = paths.reduce((acc, path) => {
    for (let i = 0; i < Math.min(acc.length, path.length); i += 1) {
      if (acc[i] !== path[i]) return acc.slice(0, i);
    }
    return acc;
  });

  const lastSlash = commonPath.lastIndexOf('/');
  return commonPath.slice(0, lastSlash + 1);
}
